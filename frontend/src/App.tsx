...
function App() {
  ...
  const handleFileUpload = async (file: File) => {
    // We don't need to call the server here anymore.
    // Just set the file in the state so the user can see it.
    try {
      setUploadedFile(file);
      setRotation(0);
      
      // Create a local preview URL for the browser to display
  ...
  const handleProcessOCR = async (compress: boolean) => {
    // 1. Check usage limits for non-premium users
    if (!userProfile.isPremium && dailyUsage >= FREE_DAILY_LIMIT) {
      toast.error('Daily limit reached. Upgrade to Premium for unlimited conversions!');
      setShowUpgrade(true);
      return;
    ...
    try {
      // 2. Prepare the data to send to Node.js
      const formData = new FormData();
      formData.append('image', uploadedFile);

      // 3. ACTUAL API CALL to your Node.js backend
      // Note: We use /api/ocr/scan which is the bridge to your Python AI
      ...
      // 4. Handle the real Amharic text coming back from Python
      if (response.data && response.data.text) {
        ...
        toast.error('OCR failed: No text returned from service');
      }
    } catch (error: any) {
      console.error('OCR processing error:', error);
      const errorMsg = error.response?.data?.error || 'AI Service is offline. Please check your Python terminal.';
